...
echo -e "\nChecking task status after pausing (should be stopped):" ps -o
pid,stat,cmd -p $task_pid # Shows the PID, status (should be T), and command
# Resuming the background task
echo -e "\nResuming the background task..."
kill -CONT $task_pid
# Verifying if the task is resumed
echo -e "\nChecking task status after resuming (should be running):" ps -o
pid,stat,cmd -p $task_pid # Shows the PID, status (should be R), and command
# Killing the background task
echo -e "\nKilling the background task..."
kill $task_pid
# Confirming the task has been terminated
echo -e "\nChecking task status after termination (should not exist):" ps -p
$task_pid && echo "Process still exists." || echo "Process terminated."
